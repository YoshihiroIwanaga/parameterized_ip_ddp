...
if __name__ == "__main__":
    ...
    org_diff_time_per_iter = np.mean(diff_time[np.where(diff_time > 1e-5)])
    ...
    zi_diff_time_per_iter = np.mean(diff_time[np.where(diff_time > 1e-5)])
    ...
    li_diff_time_per_iter = np.mean(diff_time[np.where(diff_time > 1e-5)])
    ...
    ci_diff_time_per_iter = np.mean(diff_time[np.where(diff_time > 1e-5)])
